...
public sealed record AdjustCombatModifierEffect(int Delta, string Raw) : Effect(Raw);

public sealed record EnduranceDamageEffect(int Amount, string Raw) : Effect(Raw);

public sealed record EnduranceHealEffect(int Amount, string Raw) : Effect(Raw);
...
public sealed record UpdateSlotEffect(string SlotName, int Value, bool IsAbsolute, string Raw) : Effect(Raw);
